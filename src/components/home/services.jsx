const Services=()=>{
    return(
        <>
        <section id="services">
        <div class="services_banner"><div class="servbancont"><h2 style={{margin:"0px",position:"relative",top:"182px",fontSize:"41px",color:"black",zIndex:3}}>Our Services</h2>
        <div style={{position:"relative",textAlign:"center",top:"212px",left:"107px",height:"109px",width: "1031px",fontSize:"21px",backgroundColor:"white",padding: "8px",opacity:"0.5",borderRadius:"20px"}}></div>
        <p style={{position:"relative",textAlign:"center",top:"99px",left:"92px",height:"109px",width: "1031px",fontSize:"19px",padding: "8px"}}>At Online Clinic, we offer a wide range of healthcare services to meet your needs. Our team of experienced medical professionals is dedicated to providing high-quality care in the comfort of your home.</p></div>
        </div>
        <div class="container24">
            
            <div class="services-grid">
                <div class="service-item">
                    <img width="100%" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR_x-m2Zy_tYizt-jmw_tvYiNyz4QqGGEi9EA&s" alt="Telemedicine Consultations"/>
                    <h3 style={{color:"rgb(25, 119, 204)"}}>Easy Appointment booking</h3>
                    <p style={{color:"black"}}>Consult with our doctors via secure video conferencing. Get expert medical advice without leaving your home.</p>
                </div>
                <div class="service-item">
                    <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQ1AGDe6c5p-Td2DMxoemln0LZFXdnRT9WBHQ&s" alt="Online Prescriptions"/>
                    <h3 style={{color:"rgb(25, 119, 204)"}}>Online Prescriptions</h3>
                    <p style={{color:"black"}}>Receive prescriptions online for your convenience. Our doctors will ensure you get the right medication for your condition.</p>
                </div>
                <div class="service-item">
                    <img src="https://uploads-ssl.webflow.com/5f3c19f18169b62a0d0bf387/5f3d74438aefe54b9158facc_s_B7B60683A076D6B2B6B1AAFAC63899C5CEBA7AFAAA6A7510BBAD17DFD503123E_1597094830927_national-cancer-institute-L8tWZT4CcVQ-unsplash.jpeg"/>
                    <h3 style={{color:"rgb(25, 119, 204)"}}>Chatting with doctor</h3>
                    <p style={{color:"black"}}>Manage your chronic conditions with personalized care plans and regular check-ins from our healthcare team.</p>
                </div>
            </div>
        </div>
    </section>
        </>
    )
}

export default Services